...
def _load_model():
    """Load the trained weights once and reuse them for every prediction.

    Streamlit reruns this module's callers on almost every UI interaction
    (e.g. clicking "Listen"), so re-reading the .pth file from disk and
    rebuilding the network on each rerun was adding avoidable latency.
    """
    global _model
    if _model is None:
